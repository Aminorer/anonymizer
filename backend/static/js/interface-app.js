(function () {
    ...
    createToast(message, type) {
      ...
      toast.innerHTML = `
                <div class="flex items-center">
                    ...
                    <span class="text-gray-800 flex-1">${utils.sanitizeText(
                      message
                    )}</span>
  ...
  const useEntityStore = defineStore("entities", {
    ...
    getters: {
      ...
      highConfidenceItems: (state) =>
        state.items.filter((item) => item.confidence && item.confidence >= 0.8),
    },
    actions: {
      async fetch(jobId) {
        this.loading = true;
    ...
    actions: {
      ...
      async update(jobId, group) {
        try {
          ...
          if (index !== -1)
            this.items[index] = { ...this.items[index], ...group };
  ...
  const useAppStore = defineStore("app", {
    ...
    actions: {
      ...
      performSearch(entityStore) {
        ...
        this.searchResults = entityStore.items.filter(
          (e) => e.text && e.text.toLowerCase().includes(term)
        );
  ...
  const SearchSection = {
    ...
    setup() {
      const appStore = useAppStore();
      const entityStore = useEntityStore();
      const perform = utils.debounce(
        () => appStore.performSearch(entityStore),
    ...
    const app = createApp({
      ...
      setup() {
        ...
        onMounted(() => {
          ...
          viewer = new DocumentViewer(viewerContainer, {
            zoomStep: CONFIG.ZOOM_STEP,
          ...
          watch(
            () => appStore.documentUrl,
            async (newUrl) => {
              if (newUrl) {
                console.log("Loading document:", newUrl);
                await viewer.loadDocument(newUrl);
              }
            },
            { immediate: true }
          );
